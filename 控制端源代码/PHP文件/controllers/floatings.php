...
class Floatings extends DBClass
{
  ...
  function insert()
  {
    ...
    if ($erreur == "") {
      ...
      $sql = "INSERT INTO $this->table (id,title,type, icon,url, backgroundColor,status) 
      VALUES 
      ('$id','$this->title','$this->type','$image','$this->url','$this->backgroundColor',$this->status)";
  ...
  function update()
  {
    ...
    if ($erreur == "") {
      ...
      $sql = "UPDATE $this->table SET title = '$this->title', type= '$this->type', icon= '$image',url= '$this->url',backgroundColor = '$this->backgroundColor',status= '$this->status'  WHERE id='$this->id'";
      try {
        $this->query($sql);
